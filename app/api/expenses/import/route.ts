...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Read the file into a buffer (use ArrayBuffer directly for Node 22 compat with exceljs)
    ...
    // Find the data sheet (prefer "Расходы", fall back to first sheet)
    const ws = wb.worksheets.find((s) => s.name === "Расходы") ?? wb.worksheets[0];
